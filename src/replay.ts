...
export type ProgressCallback = (stepIndex: number, totalSteps: number, message: string) => void;

// Helper to translate Puppeteer/Chrome Recorder nested selectors to the most stable single CSS or XPath selector
function getPlaywrightSelector(selectors: string[][]): string {
  ...
  if (!bestSelector && selectors.length > 0 && selectors[0].length > 0) {
    ...
    if (primary.startsWith('xpath/')) return 'xpath=' + primary.substring(6);
    if (primary.startsWith('aria/')) return `text=${primary.substring(5)}`;
    return primary;
  }
...
async function getElementMetadata(page: Page, selector: string) {
  try {
    return await page.$eval(selector, (el: HTMLElement) => {
...
export async function replaySession(
  jsonContent: string,
  sessionName: string = 'generated_test',
