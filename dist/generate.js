...
exports.compileDirectlyFromChromeRecorder = compileDirectlyFromChromeRecorder;
exports.ensureCommonStepFiles = ensureCommonStepFiles;
...
function getExistingStepDefinitionsContext() {
    const stepsDir = path.join(OUTPUT_DIR, 'steps');
    if (!fs.existsSync(stepsDir))
...
function getExistingPagesContext() {
    const pagesDir = path.join(OUTPUT_DIR, 'pages');
    if (!fs.existsSync(pagesDir))
        return 'No existing page classes found.';
    let context = '';
...
async function generateTestScript(sessionName = 'generated_test') {
...
async function compileDirectlyFromChromeRecorder(jsonContent, sessionName) {
...
`;
    ...
    const dummyActions = (flow.steps || []).map((step, index) => ({
        timestamp: Date.now(),
        step: index + 1,
        action: step.type,
        url: step.url || '',
